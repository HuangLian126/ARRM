...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        """
        Arguments:
            x (tuple[tensor, tensor]): x contains the class logits and the box_regression from the model.
            boxes (list[BoxList]): bounding boxes that are used as reference, one for each image.

        Returns:
            results (list[BoxList]): one BoxList for each image, containing the extra fields labels and scores
        """
        ...
        # print('class_prob2.shape: ', class_prob.shape)
    ...
    def filter_results(self, boxlist, num_classes):
        ...
        scores = boxlist.get_field("scores").reshape(-1, num_classes)  # [1000, 16]


        # indices = torch.arange(start=0, end=1000)  # 新加的
        # indices = indices.expand((scores.shape[1], scores.shape[0])).t().shape  # 新加的 [1000, 16]
        # print('indices444: ', indices)
        ...
        inds_all = scores > self.score_thresh  #  torch.Size([1000, 16])
